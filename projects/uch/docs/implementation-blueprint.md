...
5. **Connectome: explicit cross-reference to the cue-tag-content / active-reconstruction model**, since it's a strong conceptual match worth formalizing in the organ's own design doc. *Implemented 2026-07-31:* `design/CONNECTOME.md` formalizes the cue-tag-content / active-reconstruction model, and `src/connectome/wiring.ts` was expanded from scaffold to a full organ — weighted edges (`weight`, `maxWeight`, `rankPaths`), activation propagation (`activate()` BFS with per-hop decay, `getNeighborhood()`), auto-wiring (exoskeleton subscribes to `connectome:link` signals; boot seeds emitted as events), integrity (`checkIntegrity()`: weak connectivity, components, dangling edges, emergency-broadcast flag per FORMAL_FOUNDATIONS), and persistence (`Storable` persist/load). 20 new tests in `src/__tests__/connectome-expansion.test.ts`.
