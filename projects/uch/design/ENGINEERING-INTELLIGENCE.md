...
- **Status:** Proposed — Phase A design (see [ADR-003](ADR-003-engineering-intelligence-layer.md))
- **Date:** 2026-07-31
...
domains that *continuously evaluates and influences* any connected agent — so the
...
| VII | Architectural Patterns | "Which pattern — and when *not* to use it?" |
...
| Tier VI Laws (partial) | `src/cognitive-plane/constitution/` (governance laws), `src/kernel/constitution/` (coding guidelines, engineering-judgment, organic-score, epistemology) | **Partial** — enforcement gates exist; laws as *reasoning primitives* do not |
...
| Tier X Unknown unknowns (seeds) | `src/cognitive-plane/dreaming/`, `suggestions/`, `patterns/pattern-library.ts` (trace-pattern matching) | **Partial** — no active cross-domain unknown-unknown scan |
...
2. **Tier II — SE Intelligence**: the constitution enforces laws, but nothing
   *measures* coupling, cohesion, connascence, change amplification, cognitive
...
7. **Tier VII — Architecture-pattern trade-offs**: no catalog of when *not* to
   use each pattern.
...
| **E** | Learning loop: taste feedback, connectome weights, sleep distillation, Tier X dream scan | Distillation produces skill entries; scan finds ≥ 1 real unknown-unknown in a seeded corpus |
...
- arXiv:2310.03533 — *Large Language Models for Software Engineering: Survey and Open Problems*
- arXiv:2004.10777 — *Code Smells and Refactoring: A Tertiary Systematic Review*
- laws-of-software-engineering.com — *Laws of Software Engineering* (catalog of empirical laws)
- Manning — *The Coder Cafe* (Teiva Harsanyi) — data-structure trade-off corpus
