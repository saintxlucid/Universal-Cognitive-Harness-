...
1. **Analyze requirements** â€” Identify data sources, tools needed, and client apps
2. **Initialize project** â€” `npx @modelcontextprotocol/create-server my-server` (TypeScript) or `pip install mcp` + scaffold (Python)
3. **Design protocol** â€” Define resource URIs, tool schemas (Zod/Pydantic), and prompt templates
4. **Implement** â€” Register tools and resource handlers; configure transport (stdio/SSE/HTTP)
5. **Test** â€” Run `npx @modelcontextprotocol/inspector` to verify protocol compliance interactively; confirm tools appear, schemas accept valid inputs, and error responses are well-formed JSON-RPC 2.0. **Feedback loop:** if schema validation fails â†’ inspect Zod/Pydantic error output â†’ fix schema definition â†’ re-run inspector. If a tool call returns a malformed response â†’ check transport serialisation â†’ fix handler â†’ re-test.
6. **Deploy** â€” Package, add auth/rate-limiting, configure env vars, monitor
...
### TypeScript â€” Tool with Zod Validation
...
### Python â€” Tool with Pydantic Validation
...
```
Client â†’ { "method": "tools/call", "params": { "name": "get_weather", "arguments": { "location": "Berlin" } } }
Server â†’ { "result": { "content": [{ "type": "text", "text": "{\"temp\": 18, \"units\": \"celsius\"}" }] } }
```
