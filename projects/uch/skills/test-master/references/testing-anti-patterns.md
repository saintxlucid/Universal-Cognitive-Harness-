...
```typescript
// âŒ BAD: Testing the mock, not the behavior
...
```typescript
// âŒ BAD: Production code polluted with test concerns
class UserCache {
...
```typescript
// âœ… GOOD: Test utilities separate from production
...
```typescript
// âŒ BAD: Mocking everything without understanding
...
```typescript
// âœ… GOOD: Strategic mocking with real components where possible
...
```typescript
// âŒ BAD: Incomplete mock response
...
```typescript
// âœ… GOOD: Complete mock matching real API response
const mockUserApi = jest.fn().mockResolvedValue({
  id: 1,
  name: 'Test User',
...
```typescript
// âœ… GOOD: Tests are part of implementation
// Write failing test
