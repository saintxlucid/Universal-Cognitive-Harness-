...
1. **Analyze Performance** â€” Capture baseline metrics and run `EXPLAIN ANALYZE` before any changes
2. **Identify Bottlenecks** â€” Find inefficient queries, missing indexes, config issues
3. **Design Solutions** â€” Create index strategies, query rewrites, schema improvements
4. **Implement Changes** â€” Apply optimizations incrementally with monitoring; validate each change before proceeding to the next
5. **Validate Results** â€” Re-run `EXPLAIN ANALYZE`, compare costs, measure wall-clock improvement, document changes

> âš ï¸ Always test changes in non-production first. Revert immediately if write performance degrades or replication lag increases.
...
### Reading EXPLAIN Output â€” Key Patterns to Find
...
### MUST DO
- Capture `EXPLAIN (ANALYZE, BUFFERS)` output **before** optimizing â€” this is the baseline
